...
#ifndef TURTLEBOT3_GIX_HARDWARE__OPENCR_HPP_
#define TURTLEBOT3_GIX_HARDWARE__OPENCR_HPP_
...
#include "turtlebot3_gix_hardware/dynamixel_sdk_wrapper.hpp"
#include "turtlebot3_gix_hardware/opencr_control_table.hpp"
#include "turtlebot3_gix_hardware/opencr_definitions.hpp"
...
public:
  ...
  void gix_torque(uint8_t onoff) const;
  ...
  std::array<double, 1> get_gix_positions();
  std::array<double, 1> get_gix_velocities();

  bool set_gix_positions(const std::vector<double> & radians);
  bool set_gix_profile_acceleration(
    const std::array<int32_t, 1> & acceleration);
  bool set_gix_profile_velocity(const std::array<int32_t, 1> & velocity);
...
#endif  // TURTLEBOT3_GIX_HARDWARE__OPENCR_HPP_
